...
@test "ctr logs: journald priority level" {
    setup_container_env "for i in \$(/busybox seq 0 7); do /busybox echo \\\"<\$i>msg with priority \$i\\\"; done"
    run_conmon_with_default_args \
        --log-path "journald:"

    for i in $( seq 0 7); do
        run journalctl --user CONTAINER_ID_FULL="$CTR_ID" PRIORITY=$i
        assert "${output}" =~ "msg with priority $i"
    done
}
