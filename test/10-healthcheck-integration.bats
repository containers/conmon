...
EOF
    ...
    local conmon_pid=$(start_conmon_healthcheck "/busybox" 9 "$healthcheck_timeout" 1 0 "k8s-file:$LOG_PATH" "trace" sleep 10)
    echo "Conmon started with PID: $conmon_pid"

    # With --sync, conmon doesn't write a PID file, so find the forked conmon process
    echo "Looking for forked conmon processes..."
    local actual_conmon_pid=$(find_conmon_forked_pid "$conmon_pid" "$CTR_ID")
    echo "Using conmon PID: $actual_conmon_pid"

    # Wait for healthcheck to start and timeout
    # Healthcheck should start immediately and timeout after 2 seconds
    ...
    # Clean up
    CONMON_PROCESS_DETAILS=$(ps -ef | grep "$actual_conmon_pid")
