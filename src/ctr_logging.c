...
{
	_cleanup_(strv_cleanup) char **allowed_dirs = NULL;
	char **dir_iter;

	if (!opt_log_allowlist_dirs || strlen(opt_log_allowlist_dirs) == 0)
		return TRUE; /* No allowlist configured, allow all paths */

	/* Split allowlist by colon separator */
	allowed_dirs = g_strsplit(opt_log_allowlist_dirs, ":", -1);
	if (!allowed_dirs)
		return FALSE;

	/* Check if canonical path starts with any allowed directory */
	for (dir_iter = allowed_dirs; *dir_iter; dir_iter++) {
...
static int secure_validate_log_path(const char *path)
{
	...
	if (fstatat(parent_fd, basename, &path_stat, AT_SYMLINK_NOFOLLOW) == 0) {
		...
		if (strstr(canonical_path, "/../") || g_str_has_suffix(canonical_path, "/..") || strcmp(canonical_path, "..") == 0
		    || strstr(canonical_path, "/./")) {
			nwarnf("Parent path contains directory traversal");
			goto cleanup;
		}
