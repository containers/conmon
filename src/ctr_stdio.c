...
static bool read_stdio(int fd, stdpipe_t pipe, gboolean *eof)
{
	...
	num_read = read(fd, writev_buffer.buf, writev_buffer.buf_len);
	if (num_read == 0) {
		if (eof)
			*eof = true;
		return false;
		...
		bool written = write_to_logs(pipe, num_read);
		if (!written)
			return written;

		write_back_to_remote_consoles(pipe, writev_buffer.buf, num_read);
		return true;
	}
}
