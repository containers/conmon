...
static GOptionEntry opt_entries[] = {
    ...
    {"leave-stdin-open", 0, 0, G_OPTION_ARG_NONE, &opt_leave_stdin_open,
     "Leave stdin open when attached client disconnects", NULL},
    {"cid", 'c', 0, G_OPTION_ARG_STRING, &opt_cid, "Container ID", NULL},
    {"cuuid", 'u', 0, G_OPTION_ARG_STRING, &opt_cuuid, "Container UUID", NULL},
    {"runtime", 'r', 0, G_OPTION_ARG_STRING, &opt_runtime_path, "Runtime path",
     NULL},
    {"restore", 0, 0, G_OPTION_ARG_STRING, &opt_restore_path,
     "Restore a container from a checkpoint", NULL},
    {"no-new-keyring", 0, 0, G_OPTION_ARG_NONE, &opt_no_new_keyring,
     "Do not create a new session keyring for the container", NULL},
    {"no-pivot", 0, 0, G_OPTION_ARG_NONE, &opt_no_pivot,
     "Do not use pivot_root", NULL},
    {"replace-listen-pid", 0, 0, G_OPTION_ARG_NONE, &opt_replace_listen_pid,
     "Replace listen pid if set for oci-runtime pid", NULL},
    {"bundle", 'b', 0, G_OPTION_ARG_STRING, &opt_bundle_path, "Bundle path",
     NULL},
    {"pidfile", 0, 0, G_OPTION_ARG_STRING, &opt_container_pid_file,
     "PID file (DEPRECATED)", NULL},
    {"container-pidfile", 'p', 0, G_OPTION_ARG_STRING, &opt_container_pid_file,
     "Container PID file", NULL},
    ...
    {"systemd-cgroup", 's', 0, G_OPTION_ARG_NONE, &opt_systemd_cgroup,
     "Enable systemd cgroup manager", NULL},
    {"exec", 'e', 0, G_OPTION_ARG_NONE, &opt_exec,
     "Exec a command in a running container", NULL},
    {"exec-process-spec", 0, 0, G_OPTION_ARG_STRING, &opt_exec_process_spec,
     "Path to the process spec for exec", NULL},
    {"exit-dir", 0, 0, G_OPTION_ARG_STRING, &opt_exit_dir,
     "Path to the directory where exit files are written", NULL},
    ...
    {"timeout", 'T', 0, G_OPTION_ARG_INT, &opt_timeout, "Timeout in seconds",
     NULL},
    {"log-size-max", 0, 0, G_OPTION_ARG_INT64, &opt_log_size_max,
     "Maximum size of log file", NULL},
    {"socket-dir-path", 0, 0, G_OPTION_ARG_STRING, &opt_socket_path,
     "Location of container attach sockets", NULL},
    {"version", 0, 0, G_OPTION_ARG_NONE, &opt_version,
     "Print the version and exit", NULL},
    {"syslog", 0, 0, G_OPTION_ARG_NONE, &opt_syslog,
     "Log to syslog (use with cgroupfs cgroup manager)", NULL},
    {"log-level", 0, 0, G_OPTION_ARG_STRING, &opt_log_level,
     "Print debug logs based on log level", NULL},
    {NULL}};
...
/* Parse_level parses the string value of the --log_level flag to its matching
 * enum */
...
ssize_t writev_buffer_append_segment(int fd, writev_buffer_t *buf,
...
int set_k8s_timestamp(char *buf, ssize_t buflen, const char *pipename) {
  ...
  len =
      snprintf(buf, buflen, "%d-%02d-%02dT%02d:%02d:%02d.%09ld%c%02d:%02d %s ",
               tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour,
               tm->tm_min, tm->tm_sec, ts.tv_nsec, off_sign, off / 3600,
               (off % 3600) / 60, pipename);
 ...
 */
static int write_k8s_log(int fd, stdpipe_t pipe, const char *buf,
                         ssize_t buflen) {
  char tsbuf[TSBUFLEN];
  ...
  while (buflen > 0) {
     ...
     * a timestamp.
     */
    if ((opt_log_size_max > 0) &&
        (bytes_written + bytes_to_be_written) > opt_log_size_max) {
      bytes_written = 0;
...
static bool read_stdio(int fd, stdpipe_t pipe, gboolean *eof) {
  ...
  if (num_read == 0) {
    ...
    for (i = conn_socks->len; i > 0; i--) {
      ...
      if (conn_sock->writable &&
          write_all(conn_sock->fd, real_buf, num_read + 1) < 0) {
...
static void on_sigchld(G_GNUC_UNUSED int signal) { raise(SIGUSR1); }
...
static gboolean oom_cb(int fd, GIOCondition condition,
...
#define CONN_SOCK_BUF_SIZE                                                     \
  32 * 1024 /* Match the write size in CopyDetachable */
static gboolean conn_sock_cb(int fd, GIOCondition condition,
                             gpointer user_data) {
...
static gboolean attach_cb(int fd, G_GNUC_UNUSED GIOCondition condition,
  ...
  if (conn_fd == -1) {
    ...
    g_unix_fd_add(conn_sock->fd, G_IO_IN | G_IO_HUP | G_IO_ERR, conn_sock_cb,
                  conn_sock);
...
static gboolean ctrl_cb(int fd, G_GNUC_UNUSED GIOCondition condition,
  ...
  while (*newline != '\0') {
    ...
    ninfof("Message type: %d, Height: %d, Width: %d", ctl_msg_type, height,
           width);
    switch (ctl_msg_type) {
...
static gboolean terminal_accept_cb(int fd, G_GNUC_UNUSED GIOCondition condition,
                                   G_GNUC_UNUSED gpointer user_data) {
...
static void runtime_exit_cb(G_GNUC_UNUSED GPid pid, int status,
                            G_GNUC_UNUSED gpointer user_data) {
...
static void container_exit_cb(G_GNUC_UNUSED GPid pid, int status,
                              G_GNUC_UNUSED gpointer user_data) {
...
static char *setup_console_socket(void) {
  ...
  _cleanup_free_ char *csname =
      g_build_filename(tmpdir, "conmon-term.XXXXXX", NULL);
  /*
...
static char *setup_attach_socket(void) {
  ...
  attach_sock_path =
      g_build_filename(opt_socket_path, opt_cuuid, "attach", NULL);
  ninfof("attach sock path: %s", attach_sock_path);

  strncpy(attach_addr.sun_path, attach_sock_path,
          sizeof(attach_addr.sun_path) - 1);
  ninfof("addr{sun_family=AF_UNIX, sun_path=%s}", attach_addr.sun_path);

  /*
   * We make the socket non-blocking to avoid a race where client aborts
   * connection before the server gets a chance to call accept. In that
   * scenario, the server accept blocks till a new client connection comes in.
   */
  attach_socket_fd =
...
static void setup_terminal_control_fifo() {
  _cleanup_free_ char *ctl_fifo_path =
      g_build_filename(opt_bundle_path, "ctl", NULL);
...
static void setup_oom_handling(int container_pid) {
  ...
  _cleanup_free_ char *memory_cgroup_path =
      process_cgroup_subsystem_path(container_pid, "memory");
  ...
  _cleanup_free_ char *memory_cgroup_file_oom_path =
      g_build_filename(memory_cgroup_path, "memory.oom_control", NULL);
...
int main(int argc, char *argv[]) {
  ...
  if (opt_exec) {
    add_argv(runtime_argv, "exec", "-d", "--pid-file", opt_container_pid_file,
             NULL);
  } else {
    char *command;
    if (opt_restore_path)
    ...
    add_argv(runtime_argv, command, "--bundle", opt_bundle_path, "--pid-file",
             opt_container_pid_file, NULL);
    ...
    if (opt_restore_path) {
       ...
       */
      add_argv(runtime_argv, "--detach", "--image-path", opt_restore_path,
               "--work-path", opt_bundle_path, NULL);
    }
  }
  ...
  if ((sigemptyset(&mask) < 0) || (sigaddset(&mask, SIGTERM) < 0) ||
      (sigaddset(&mask, SIGQUIT) < 0) || (sigaddset(&mask, SIGINT) < 0) ||
  ...
  if (create_pid < 0) {
    ...
    /* FIXME: This results in us not outputting runc error messages to crio's
     * log. */
  ...
  if ((signal(SIGTERM, on_sig_exit) == SIG_ERR) ||
      (signal(SIGQUIT, on_sig_exit) == SIG_ERR) ||
  ...
  /*
   * Glib does not support SIGCHLD so use SIGUSR1 with the same semantic.  We
   * will catch SIGCHLD and raise(SIGUSR1) in the signal handler.
   */
  ...
  if (csname != NULL) {
    guint terminal_watch =
        g_unix_fd_add(console_socket_fd, G_IO_IN, terminal_accept_cb, csname);
    /* Process any SIGCHLD we may have missed before the signal handler was in
     * place.  */
    check_child_processes(pid_to_handler);
  ...
  g_hash_table_insert(pid_to_handler, (pid_t *)&container_pid,
                      container_exit_cb);
  ...
  /*
   * Podman injects some fd's into the conmon process so that exposed ports are
   * kept busy while the container runs.  Close them before we notify the
   * container exited, so that they can be reused immediately.
   */
  for (fd = 3;; fd++) {
